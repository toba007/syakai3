...
class FaceAnimationApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Face Animation")
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(controls, text="GIF保存", command=self.export_gif).grid(row=0, column=2, padx=(0, 16))

        ttk.Label(controls, text="FPS").grid(row=0, column=3, padx=(0, 4))
        ttk.Spinbox(controls, from_=3, to=30, textvariable=self.fps, width=5).grid(row=0, column=4, padx=(0, 12))

        ttk.Label(controls, text="秒").grid(row=0, column=5, padx=(0, 4))
        ttk.Spinbox(controls, from_=1, to=60, increment=1, textvariable=self.gif_seconds, width=5).grid(
            row=0, column=6, padx=(0, 12)
        )

        ttk.Label(controls, text="サイズ").grid(row=0, column=7, padx=(0, 4))
        ttk.Spinbox(controls, from_=256, to=1024, increment=64, textvariable=self.size, width=6).grid(
            row=0, column=8, padx=(0, 12)
        )

        ttk.Label(controls, text="Seed").grid(row=0, column=9, padx=(0, 4))
        ttk.Entry(controls, textvariable=self.seed, width=8).grid(row=0, column=10)
    ...
    def _force_emotion(self, emotion: str) -> None:
        self.forced_emotion = emotion
        self.forced_emotion_until = time.monotonic() + 3.0
